...
/**
 * `/pl/community` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/community/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/pl/community' })`, breadcrumb `Home` → `/pl`,
 ...
 * `/pl/community` and `alternates.languages` `pl` + `en` + `x-default` → EN canonical.
 */
...
export default function PlCommunityPage() {
  return (
    <>
      ...
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/pl' },
          { name: 'Community', path: '/pl/community' },
        ])}
      />
    </>
  );

...
/**
 * `/pl/location/[country]` — generated locale location
 ...
 * `/pl/location/[country]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface PlCountryPageProps {
  params: Promise<{ country: string }>;
}

export async function generateMetadata({ params }: PlCountryPageProps): Promise<Metadata> {
  const { country } = await params;
  const localeEntry = resolveLocationEntry({ country }, 'pl');
...
export default async function PlCountryPage({ params }: PlCountryPageProps) {
  const { country } = await params;

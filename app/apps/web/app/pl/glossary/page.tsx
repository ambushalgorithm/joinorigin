...
/**
 * `/pl/glossary` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/glossary/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/pl/glossary' })`, breadcrumb `Home` → `/pl`,
 ...
 * `/pl/glossary` and `alternates.languages` `pl` + `en` + `x-default` → EN canonical.
 */
...
export default function PlGlossaryPage() {
  return (
    <>
      <GlossaryHubView />
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/pl' },
          { name: 'Glossary', path: '/pl/glossary' },
        ])}
      />
    </>
  );

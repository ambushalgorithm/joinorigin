...
/**
 * `/ar/location/[country]` — generated locale location
 ...
 * (`resolveLocationEntry(params, 'ar')`), EN entry otherwise —
 ...
 * `/ar/location/[country]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface ArCountryPageProps {
  params: Promise<{ country: string }>;
}

export async function generateMetadata({ params }: ArCountryPageProps): Promise<Metadata> {
  const { country } = await params;
  const localeEntry = resolveLocationEntry({ country }, 'ar');
  ...
  return localeEntry
    ...
    : localizeMetadata(locationMetadata(entry), 'ar', entry.path);
}

export default async function ArCountryPage({ params }: ArCountryPageProps) {
  const { country } = await params;

...
/**
 * `/ar/community` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/community/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/ar/community' })`, breadcrumb `Home` → `/ar`,
 ...
 * `/ar/community` and `alternates.languages` `ar` + `en` + `x-default` → EN canonical.
 */
...
export default function ArCommunityPage() {
  return (
    <>
      ...
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/ar' },
          { name: 'Community', path: '/ar/community' },
        ])}
      />
    </>
  );

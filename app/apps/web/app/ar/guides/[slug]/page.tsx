...
export function generateStaticParams() {
  return guidePageEntries('ar').map((entry) => ({ slug: entry.slug }));
}
...
export async function generateMetadata({
  params,
}: ArGuidePageProps): Promise<Metadata> {
  const { slug } = await params;
  const entry = guidePageEntry(slug, 'ar');
  if (!entry) {
    return {};
  }
...
export default async function ArGuidePage({ params }: ArGuidePageProps) {
  const { slug } = await params;
  const page = guidePageForLocale(slug, 'ar');
  if (!page) {
    notFound();
  }

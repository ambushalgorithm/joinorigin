 ...
 * `/ar/guides` with `x-default` → EN canonical
 * (`guideHubMetadata`). The visible FAQ (G-12) resolves per-locale via
 * `guideHubFaq` and is mirrored 1:1 in the `FAQPage` JSON-LD.
 */
export const metadata: Metadata = guideHubMetadata('ar');

export default function ArGuidesHubPage() {
  const entries = guidePageEntriesWithFallback('ar');
  const faq = guideHubFaq('ar');
  return (
    <>

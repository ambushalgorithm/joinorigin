...
/**
 * `/zh-CN/location` — generated locale location hub (TASK-448).
 *
 ...
 * while canonical + hreflang localize to `/zh-CN/location` with
...
export const metadata: Metadata = (() => {
  ...
  return localizeMetadata(locationMetadata(entry), 'zh-CN', entry.path);
})();

export default async function ZhCNLocationHubPage() {
  const entry = hubEntry();
  if (!entry) {
    return null;
  }
  const data = buildLocationViewData(entry, 'zh-CN', await getServerCountry());

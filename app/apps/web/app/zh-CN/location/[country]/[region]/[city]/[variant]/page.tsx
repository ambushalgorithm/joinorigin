...
/**
 * `/zh-CN/location/[country]/[region]/[city]/[variant]` — generated locale location
 ...
 * `/zh-CN/location/[country]/[region]/[city]/[variant]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface ZhCNVariantPageProps {
  params: Promise<{ country: string; region: string; city: string; variant: string }>;
}

export async function generateMetadata({ params }: ZhCNVariantPageProps): Promise<Metadata> {
  const { country, region, city, variant } = await params;
  const localeEntry = resolveLocationEntry({ country, region, city, variant }, 'zh-CN');
...
export default async function ZhCNVariantPage({ params }: ZhCNVariantPageProps) {
  const { country, region, city, variant } = await params;

 ...
 * `/zh-CN/guides/[slug]` with `x-default` → EN canonical.
 */
...
export async function generateMetadata({ params }: ZhCNGuidePageProps): Promise<Metadata> {
  ...
  const entry = guidePageEntry(slug, 'zh-CN') ?? guidePageEntry(slug);
  if (!entry) {
    return {};
  }
  return guidePageMetadata(entry, 'zh-CN');
}

export default async function ZhCNGuidePage({ params }: ZhCNGuidePageProps) {
  const { slug } = await params;
  const page = guidePageForLocale(slug, 'zh-CN') ?? guidePageForLocale(slug);
  if (!page) {
    notFound();
  }

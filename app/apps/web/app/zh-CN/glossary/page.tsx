...
/**
 * `/zh-CN/glossary` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/glossary/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/zh-CN/glossary' })`, breadcrumb `Home` → `/zh-CN`,
 ...
 * `/zh-CN/glossary` and `alternates.languages` `zh-CN` + `en` + `x-default` → EN canonical.
 */
...
export default function ZhCNGlossaryPage() {
  return (
    <>
      <GlossaryHubView />
      <JsonLd
        data={breadcrumbList([
          ...
          { name: 'Glossary', path: '/zh-CN/glossary' },
        ])}
      />
    </>
  );

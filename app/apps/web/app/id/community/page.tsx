...
/**
 * `/id/community` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/community/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/id/community' })`, breadcrumb `Home` → `/id`,
 ...
 * `/id/community` and `alternates.languages` `id` + `en` + `x-default` → EN canonical.
 */
...
export default function IdCommunityPage() {
  return (
    <>
      <CommunityView />
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/id' },
          { name: 'Community', path: '/id/community' },
        ])}
      />
    </>
  );

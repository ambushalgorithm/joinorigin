 ...
 * `/id/guides` with `x-default` → EN canonical
 * (`guideHubMetadata`). The visible FAQ (G-12) resolves per-locale via
 * `guideHubFaq` and is mirrored 1:1 in the `FAQPage` JSON-LD.
 */
export const metadata: Metadata = guideHubMetadata('id');

export default function IdGuidesHubPage() {
  const entries = guidePageEntriesWithFallback('id');
  const faq = guideHubFaq('id');
  return (
    <>

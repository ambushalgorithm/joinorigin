 ...
 * `/id/guides/[slug]` with `x-default` → EN canonical.
 */
...
interface IdGuidePageProps {
  params: Promise<{ slug: string }>;
}

export async function generateMetadata({ params }: IdGuidePageProps): Promise<Metadata> {
  const { slug } = await params;
  const entry = guidePageEntry(slug, 'id') ?? guidePageEntry(slug);
  if (!entry) {
    return {};
  }
  return guidePageMetadata(entry, 'id');
}

export default async function IdGuidePage({ params }: IdGuidePageProps) {
  const { slug } = await params;
  const page = guidePageForLocale(slug, 'id') ?? guidePageForLocale(slug);
  if (!page) {
    notFound();
  }
  ...
  return (
    <>
      ...
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/id' },
          { name: 'Guides', path: guideHubPath('id') },

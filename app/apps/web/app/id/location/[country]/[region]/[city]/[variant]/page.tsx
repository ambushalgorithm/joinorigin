...
/**
 * `/id/location/[country]/[region]/[city]/[variant]` — generated locale location
 ...
 * (`resolveLocationEntry(params, 'id')`), EN entry otherwise —
 ...
 * `/id/location/[country]/[region]/[city]/[variant]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface IdVariantPageProps {
  params: Promise<{ country: string; region: string; city: string; variant: string }>;
}

export async function generateMetadata({ params }: IdVariantPageProps): Promise<Metadata> {
  const { country, region, city, variant } = await params;
  const localeEntry = resolveLocationEntry({ country, region, city, variant }, 'id');
  ...
  return localeEntry
    ...
    : localizeMetadata(locationMetadata(entry), 'id', entry.path);
}

export default async function IdVariantPage({ params }: IdVariantPageProps) {
  const { country, region, city, variant } = await params;

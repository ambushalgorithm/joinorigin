...
/**
 * `/id/location/[country]` — generated locale location
 ...
 * `/id/location/[country]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface IdCountryPageProps {
  params: Promise<{ country: string }>;
}

export async function generateMetadata({ params }: IdCountryPageProps): Promise<Metadata> {
  const { country } = await params;
  const localeEntry = resolveLocationEntry({ country }, 'id');
...
export default async function IdCountryPage({ params }: IdCountryPageProps) {
  const { country } = await params;

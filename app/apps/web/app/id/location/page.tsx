...
/**
 * `/id/location` — generated locale location hub (TASK-448).
 *
 ...
 * while canonical + hreflang localize to `/id/location` with
...
export const metadata: Metadata = (() => {
  ...
  return localizeMetadata(locationMetadata(entry), 'id', entry.path);
})();

export default async function IdLocationHubPage() {
  const entry = hubEntry();
  if (!entry) {
    return null;
  }
  const data = buildLocationViewData(entry, 'id', await getServerCountry());

...
describe('app/sitemap — hreflang clusters across all 21 locale surfaces', () => {
  ...
  it('emits every indexable de Berlin page as its own indexable URL with a full cluster', () => {
    ...
    for (const entry of deEntries) {
      ...
      // de self + en alternate (at /en/**) + x-default → EN canonical.
      expect(languages?.['de']).toBe(absoluteUrl(entry.path));
      expect(languages?.['en']).toBe(absoluteUrl(entry.path.replace(/^\/de/, '/en')));
      expect(languages?.['x-default']).toBe(absoluteUrl(entry.path.replace(/^\/de/, '/en')));
    }
  });

  it('every non-EN location/guide page carries self + en + x-default → EN canonical at /en/', () => {
    for (const locale of SUPPORTED_LOCALES) {
      ...
      for (const entry of [...indexableLocationEntries(locale), ...guidePageEntries(locale)]) {
        ...
        expect(languages).toBeDefined();
        const enPath = entry.path.replace(new RegExp(`^/${locale}`), '/en');
  ...
  it('every non-EN static page + guides hub carries self + en + x-default → EN canonical at /en/', () => {
    for (const locale of SUPPORTED_LOCALES) {
      if (locale === 'en') continue;
      for (const route of ROUTES) {
        const path = prefixedPath(locale, route.path);
        ...
        expect(languages?.['en']).toBe(absoluteUrl(prefixedPath('en', route.path)));
      ...
      for (const path of [prefixedPath(locale, '/glossary'), prefixedPath(locale, '/guides')]) {
        ...
        expect(languages).toBeDefined();
        expect(languages?.[locale]).toBe(absoluteUrl(path));
        expect(languages?.['en']).toBe(absoluteUrl(path.replace(new RegExp(`^/${locale}`), '/en')));
        expect(languages?.['x-default']).toBe(
          absoluteUrl(path.replace(new RegExp(`^/${locale}`), '/en')),
        );
      }
    }
  });
  ...
  it('EN static routes, guides, and hubs carry the full 21-locale cluster', () => {
    for (const route of ROUTES) {
      const languages = languageKeys(byPath.get(prefixedPath('en', route.path)) as SitemapEntry);
      expect(languages).toBeDefined();
      for (const locale of SUPPORTED_LOCALES) {
    ...
    for (const entry of guidePageEntries('en')) {
      const languages = languageKeys(byPath.get(entry.path) as SitemapEntry);
      expect(languages).toBeDefined();
      expect(languages).toEqual(guideLanguagesFor(entry.slug, 'en'));
    }
  });
...
describe('app/sitemap — changeFrequency/priority preserved per surface', () => {
  ...
  it('location hub is weekly; country/region/city/variant/ideas are monthly', () => {
    expect(byPath.get('/en/location')?.changeFrequency).toBe('weekly');
  ...
  it('location priorities match the registry (hub 0.9 → variant 0.4)', () => {
    ...
    expect(byPath.get('/en/location')?.priority).toBe(0.9);

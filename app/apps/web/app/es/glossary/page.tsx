...
/**
 * `/es/glossary` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/glossary/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/es/glossary' })`, breadcrumb `Home` → `/es`,
 ...
 * `/es/glossary` and `alternates.languages` `es` + `en` + `x-default` → EN canonical.
 */
...
export default function EsGlossaryPage() {
  return (
    <>
      <GlossaryHubView />
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/es' },
          { name: 'Glossary', path: '/es/glossary' },
        ])}
      />
    </>
  );

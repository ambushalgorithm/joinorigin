...
/**
 * `/ru/location/[country]/[region]` — generated locale location
 ...
 * `/ru/location/[country]/[region]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface RuRegionPageProps {
  params: Promise<{ country: string; region: string }>;
}

export async function generateMetadata({ params }: RuRegionPageProps): Promise<Metadata> {
  const { country, region } = await params;
  const localeEntry = resolveLocationEntry({ country, region }, 'ru');
...
export default async function RuRegionPage({ params }: RuRegionPageProps) {
  const { country, region } = await params;

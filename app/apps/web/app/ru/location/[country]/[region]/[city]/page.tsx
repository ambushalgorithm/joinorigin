...
/**
 * `/ru/location/[country]/[region]/[city]` — generated locale location
 ...
 * `/ru/location/[country]/[region]/[city]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface RuCityPageProps {
  params: Promise<{ country: string; region: string; city: string }>;
}

export async function generateMetadata({ params }: RuCityPageProps): Promise<Metadata> {
  const { country, region, city } = await params;
  const localeEntry = resolveLocationEntry({ country, region, city }, 'ru');
...
export default async function RuCityPage({ params }: RuCityPageProps) {
  const { country, region, city } = await params;

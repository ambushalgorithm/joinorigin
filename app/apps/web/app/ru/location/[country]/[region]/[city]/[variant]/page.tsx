...
/**
 * `/ru/location/[country]/[region]/[city]/[variant]` — generated locale location
 ...
 * (`resolveLocationEntry(params, 'ru')`), EN entry otherwise —
 ...
 * `/ru/location/[country]/[region]/[city]/[variant]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface RuVariantPageProps {
  params: Promise<{ country: string; region: string; city: string; variant: string }>;
}

export async function generateMetadata({ params }: RuVariantPageProps): Promise<Metadata> {
  const { country, region, city, variant } = await params;
  const localeEntry = resolveLocationEntry({ country, region, city, variant }, 'ru');
  ...
  return localeEntry
    ...
    : localizeMetadata(locationMetadata(entry), 'ru', entry.path);
}

export default async function RuVariantPage({ params }: RuVariantPageProps) {
  const { country, region, city, variant } = await params;

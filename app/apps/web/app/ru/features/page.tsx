...
/**
 * `/ru/features` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/features/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/ru/features' })`, breadcrumb `Home` → `/ru`,
 ...
 * `/ru/features` and `alternates.languages` `ru` + `en` + `x-default` → EN canonical.
 */
...
export default function RuFeaturesPage() {
  return (
    <>
      <FeaturesView />
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/ru' },
          { name: 'Features', path: '/ru/features' },
        ])}
      />
    </>
  );

...
/**
 * `/en/location/[country]/[region]/[city]/[variant]` — generated locale location
 ...
 * (`resolveLocationEntry(params, 'en')`), EN entry otherwise —
 ...
 * `/en/location/[country]/[region]/[city]/[variant]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface EnVariantPageProps {
  params: Promise<{ country: string; region: string; city: string; variant: string }>;
}

export async function generateMetadata({ params }: EnVariantPageProps): Promise<Metadata> {
  const { country, region, city, variant } = await params;
  const localeEntry = resolveLocationEntry({ country, region, city, variant }, 'en');
  ...
  return localeEntry
    ...
    : localizeMetadata(locationMetadata(entry), 'en', entry.path);
}

export default async function EnVariantPage({ params }: EnVariantPageProps) {
  const { country, region, city, variant } = await params;

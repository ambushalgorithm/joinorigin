 ...
 * while canonical + hreflang localize to `/ja/location` with
...
export const metadata: Metadata = (() => {
  ...
  return localizeMetadata(locationMetadata(entry), 'ja', entry.path);
})();

export default async function JaLocationHubPage() {
  const entry = hubEntry();
  if (!entry) {
    return null;
  }
  const data = buildLocationViewData(entry, 'ja', await getServerCountry());

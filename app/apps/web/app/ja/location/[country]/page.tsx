...
/**
 * `/ja/location/[country]` — generated locale location
 ...
 * `/ja/location/[country]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface JaCountryPageProps {
  params: Promise<{ country: string }>;
}

export async function generateMetadata({ params }: JaCountryPageProps): Promise<Metadata> {
  const { country } = await params;
  const localeEntry = resolveLocationEntry({ country }, 'ja');
...
export default async function JaCountryPage({ params }: JaCountryPageProps) {
  const { country } = await params;

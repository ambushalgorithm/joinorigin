...
/**
 * `/ja/location/[country]/[region]` — generated locale location
 ...
 * `/ja/location/[country]/[region]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface JaRegionPageProps {
  params: Promise<{ country: string; region: string }>;
}

export async function generateMetadata({ params }: JaRegionPageProps): Promise<Metadata> {
  const { country, region } = await params;
  const localeEntry = resolveLocationEntry({ country, region }, 'ja');
...
export default async function JaRegionPage({ params }: JaRegionPageProps) {
  const { country, region } = await params;

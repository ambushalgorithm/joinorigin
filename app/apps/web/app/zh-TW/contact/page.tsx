...
/**
 * `/zh-TW/contact` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/contact/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/zh-TW/contact' })`, breadcrumb `Home` → `/zh-TW`,
 ...
 * `/zh-TW/contact` and `alternates.languages` `zh-TW` + `en` + `x-default` → EN canonical.
 */
...
export default function ZhTWContactPage() {
  return (
    <>
      <ContactView />
      ...
      <JsonLd
        data={breadcrumbList([
          ...
          { name: 'Contact', path: '/zh-TW/contact' },
        ])}
      />
    </>
  );

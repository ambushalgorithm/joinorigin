...
/**
 * `/zh-TW/location/[country]/[region]/[city]/[variant]` — generated locale location
 ...
 * (`resolveLocationEntry(params, 'zh-TW')`), EN entry otherwise —
 ...
 * `/zh-TW/location/[country]/[region]/[city]/[variant]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface ZhTWVariantPageProps {
  params: Promise<{ country: string; region: string; city: string; variant: string }>;
}

export async function generateMetadata({ params }: ZhTWVariantPageProps): Promise<Metadata> {
  const { country, region, city, variant } = await params;
  const localeEntry = resolveLocationEntry({ country, region, city, variant }, 'zh-TW');
  ...
  return localeEntry
    ...
    : localizeMetadata(locationMetadata(entry), 'zh-TW', entry.path);
}

export default async function ZhTWVariantPage({ params }: ZhTWVariantPageProps) {
  const { country, region, city, variant } = await params;

...
/**
 * `/zh-TW/location` — generated locale location hub (TASK-448).
 *
 ...
 * while canonical + hreflang localize to `/zh-TW/location` with
...
export const metadata: Metadata = (() => {
  ...
  return localizeMetadata(locationMetadata(entry), 'zh-TW', entry.path);
})();

export default async function ZhTWLocationHubPage() {
  const entry = hubEntry();
  if (!entry) {
    return null;
  }
  const data = buildLocationViewData(entry, 'zh-TW', await getServerCountry());

...
/**
 * `/zh-TW/network` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/network/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/zh-TW/network' })`, breadcrumb `Home` → `/zh-TW`,
 ...
 * `/zh-TW/network` and `alternates.languages` `zh-TW` + `en` + `x-default` → EN canonical.
 */
...
export default function ZhTWNetworkPage() {
  return (
    <>
      ...
      <JsonLd
        data={breadcrumbList([
          ...
          { name: 'Network', path: '/zh-TW/network' },
        ])}
      />
    </>
  );

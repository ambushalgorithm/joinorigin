...
/**
 * `/zh-TW/community` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/community/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/zh-TW/community' })`, breadcrumb `Home` → `/zh-TW`,
 ...
 * `/zh-TW/community` and `alternates.languages` `zh-TW` + `en` + `x-default` → EN canonical.
 */
...
export default function ZhTWCommunityPage() {
  return (
    <>
      ...
      <JsonLd
        data={breadcrumbList([
          ...
          { name: 'Community', path: '/zh-TW/community' },
        ])}
      />
    </>
  );

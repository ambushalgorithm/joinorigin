...
/**
 * `/fr/community` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/community/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/fr/community' })`, breadcrumb `Home` → `/fr`,
 ...
 * `/fr/community` and `alternates.languages` `fr` + `en` + `x-default` → EN canonical.
 */
  ...
  return (
    <>
      ...
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/fr' },
          { name: 'Community', path: '/fr/community' },
        ])}
      />
    </>
  );

...
/**
 * `/fr/location/[country]/[region]` — generated locale location
 ...
 * (`resolveLocationEntry(params, 'fr')`), EN entry otherwise —
 ...
 * `/fr/location/[country]/[region]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface FrRegionPageProps {
  params: Promise<{ country: string; region: string }>;
}

export async function generateMetadata({ params }: FrRegionPageProps): Promise<Metadata> {
  const { country, region } = await params;
  const localeEntry = resolveLocationEntry({ country, region }, 'fr');
  ...
  return localeEntry
    ...
    : localizeMetadata(locationMetadata(entry), 'fr', entry.path);
}

export default async function FrRegionPage({ params }: FrRegionPageProps) {
  const { country, region } = await params;

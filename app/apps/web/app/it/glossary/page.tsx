...
/**
 * `/it/glossary` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/glossary/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/it/glossary' })`, breadcrumb `Home` → `/it`,
 ...
 * `/it/glossary` and `alternates.languages` `it` + `en` + `x-default` → EN canonical.
 */
...
export default function ItGlossaryPage() {
  return (
    <>
      <GlossaryHubView />
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/it' },
          { name: 'Glossary', path: '/it/glossary' },
        ])}
      />
    </>
  );

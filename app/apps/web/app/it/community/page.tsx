...
/**
 * `/it/community` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/community/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/it/community' })`, breadcrumb `Home` → `/it`,
 ...
 * `/it/community` and `alternates.languages` `it` + `en` + `x-default` → EN canonical.
 */
  ...
  return (
    <>
      ...
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/it' },
          { name: 'Community', path: '/it/community' },
        ])}
      />
    </>
  );

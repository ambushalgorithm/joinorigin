...
/**
 * `/it/location/[country]` — generated locale location
 ...
 * `/it/location/[country]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface ItCountryPageProps {
  params: Promise<{ country: string }>;
}

export async function generateMetadata({ params }: ItCountryPageProps): Promise<Metadata> {
  const { country } = await params;
  const localeEntry = resolveLocationEntry({ country }, 'it');
...
export default async function ItCountryPage({ params }: ItCountryPageProps) {
  const { country } = await params;

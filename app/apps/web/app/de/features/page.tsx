...
/**
 * `/de/features` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/features/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/de/features' })`, breadcrumb `Home` → `/de`,
 ...
 * `/de/features` and `alternates.languages` `de` + `en` + `x-default` → EN canonical.
 */
...
export default function DeFeaturesPage() {
  return (
    <>
      <FeaturesView />
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/de' },
          { name: 'Features', path: '/de/features' },
        ])}
      />
    </>
  );

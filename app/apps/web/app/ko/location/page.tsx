...
/**
 * `/ko/location` — generated locale location hub (TASK-448).
 *
 ...
 * while canonical + hreflang localize to `/ko/location` with
...
export const metadata: Metadata = (() => {
  ...
  return localizeMetadata(locationMetadata(entry), 'ko', entry.path);
})();

export default async function KoLocationHubPage() {
  const entry = hubEntry();
  if (!entry) {
    return null;
  }
  const data = buildLocationViewData(entry, 'ko', await getServerCountry());

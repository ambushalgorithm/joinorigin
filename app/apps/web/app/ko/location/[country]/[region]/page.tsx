...
/**
 * `/ko/location/[country]/[region]` — generated locale location
 ...
 * `/ko/location/[country]/[region]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface KoRegionPageProps {
  params: Promise<{ country: string; region: string }>;
}

export async function generateMetadata({ params }: KoRegionPageProps): Promise<Metadata> {
  const { country, region } = await params;
  const localeEntry = resolveLocationEntry({ country, region }, 'ko');
...
export default async function KoRegionPage({ params }: KoRegionPageProps) {
  const { country, region } = await params;

...
/**
 * `/ko/location/[country]/[region]/[city]` — generated locale location
 ...
 * `/ko/location/[country]/[region]/[city]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface KoCityPageProps {
  params: Promise<{ country: string; region: string; city: string }>;
}

export async function generateMetadata({ params }: KoCityPageProps): Promise<Metadata> {
  const { country, region, city } = await params;
  const localeEntry = resolveLocationEntry({ country, region, city }, 'ko');
...
export default async function KoCityPage({ params }: KoCityPageProps) {
  const { country, region, city } = await params;

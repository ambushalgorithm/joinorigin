...
/**
 * `/nl/community` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/community/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/nl/community' })`, breadcrumb `Home` → `/nl`,
 ...
 * `/nl/community` and `alternates.languages` `nl` + `en` + `x-default` → EN canonical.
 */
...
export default function NlCommunityPage() {
  return (
    <>
      ...
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/nl' },
          { name: 'Community', path: '/nl/community' },
        ])}
      />
    </>
  );

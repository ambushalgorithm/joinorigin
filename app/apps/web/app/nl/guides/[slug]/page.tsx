 ...
 * `/nl/guides/[slug]` with `x-default` → EN canonical.
 */
...
interface NlGuidePageProps {
  params: Promise<{ slug: string }>;
}

export async function generateMetadata({ params }: NlGuidePageProps): Promise<Metadata> {
  const { slug } = await params;
  const entry = guidePageEntry(slug, 'nl') ?? guidePageEntry(slug);
  if (!entry) {
    return {};
  }
  return guidePageMetadata(entry, 'nl');
}

export default async function NlGuidePage({ params }: NlGuidePageProps) {
  const { slug } = await params;
  const page = guidePageForLocale(slug, 'nl') ?? guidePageForLocale(slug);
  if (!page) {
    notFound();
  }

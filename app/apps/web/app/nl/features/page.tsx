...
/**
 * `/nl/features` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/features/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/nl/features' })`, breadcrumb `Home` → `/nl`,
 ...
 * `/nl/features` and `alternates.languages` `nl` + `en` + `x-default` → EN canonical.
 */
  ...
  return (
    <>
      ...
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/nl' },
          { name: 'Features', path: '/nl/features' },
        ])}
      />
    </>
  );

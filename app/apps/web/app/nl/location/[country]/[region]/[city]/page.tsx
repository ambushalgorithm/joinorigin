...
/**
 * `/nl/location/[country]/[region]/[city]` — generated locale location
 ...
 * (`resolveLocationEntry(params, 'nl')`), EN entry otherwise —
 ...
 * `/nl/location/[country]/[region]/[city]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface NlCityPageProps {
  params: Promise<{ country: string; region: string; city: string }>;
}

export async function generateMetadata({ params }: NlCityPageProps): Promise<Metadata> {
  const { country, region, city } = await params;
  const localeEntry = resolveLocationEntry({ country, region, city }, 'nl');
  ...
  return localeEntry
    ...
    : localizeMetadata(locationMetadata(entry), 'nl', entry.path);
}

export default async function NlCityPage({ params }: NlCityPageProps) {
  const { country, region, city } = await params;

...
/**
 * `/nl/location/[country]/[region]` — generated locale location
 ...
 * `/nl/location/[country]/[region]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface NlRegionPageProps {
  params: Promise<{ country: string; region: string }>;
}

export async function generateMetadata({ params }: NlRegionPageProps): Promise<Metadata> {
  const { country, region } = await params;
  const localeEntry = resolveLocationEntry({ country, region }, 'nl');
...
export default async function NlRegionPage({ params }: NlRegionPageProps) {
  const { country, region } = await params;

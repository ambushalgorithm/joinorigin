...
/**
 * `/nl/glossary` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/glossary/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/nl/glossary' })`, breadcrumb `Home` → `/nl`,
 ...
 * `/nl/glossary` and `alternates.languages` `nl` + `en` + `x-default` → EN canonical.
 */
...
export default function NlGlossaryPage() {
  return (
    <>
      <GlossaryHubView />
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/nl' },
          { name: 'Glossary', path: '/nl/glossary' },
        ])}
      />
    </>
  );

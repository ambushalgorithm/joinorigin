...
/**
 * `/fa/location/[country]` — generated locale location
 ...
 * `/fa/location/[country]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface FaCountryPageProps {
  params: Promise<{ country: string }>;
}

export async function generateMetadata({ params }: FaCountryPageProps): Promise<Metadata> {
  const { country } = await params;
  const localeEntry = resolveLocationEntry({ country }, 'fa');
...
export default async function FaCountryPage({ params }: FaCountryPageProps) {
  const { country } = await params;

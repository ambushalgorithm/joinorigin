...
/**
 * `/fa/location/[country]/[region]` — generated locale location
 ...
 * `/fa/location/[country]/[region]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface FaRegionPageProps {
  params: Promise<{ country: string; region: string }>;
}

export async function generateMetadata({ params }: FaRegionPageProps): Promise<Metadata> {
  const { country, region } = await params;
  const localeEntry = resolveLocationEntry({ country, region }, 'fa');
...
export default async function FaRegionPage({ params }: FaRegionPageProps) {
  const { country, region } = await params;

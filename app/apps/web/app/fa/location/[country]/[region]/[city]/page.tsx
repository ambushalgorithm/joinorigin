...
/**
 * `/fa/location/[country]/[region]/[city]` — generated locale location
 ...
 * `/fa/location/[country]/[region]/[city]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface FaCityPageProps {
  params: Promise<{ country: string; region: string; city: string }>;
}

export async function generateMetadata({ params }: FaCityPageProps): Promise<Metadata> {
  const { country, region, city } = await params;
  const localeEntry = resolveLocationEntry({ country, region, city }, 'fa');
...
export default async function FaCityPage({ params }: FaCityPageProps) {
  const { country, region, city } = await params;

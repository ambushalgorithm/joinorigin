...
/**
 * `/fa/location/[country]/[region]/[city]/[variant]` — generated locale location
 ...
 * (`resolveLocationEntry(params, 'fa')`), EN entry otherwise —
 ...
 * `/fa/location/[country]/[region]/[city]/[variant]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface FaVariantPageProps {
  params: Promise<{ country: string; region: string; city: string; variant: string }>;
}

export async function generateMetadata({ params }: FaVariantPageProps): Promise<Metadata> {
  const { country, region, city, variant } = await params;
  const localeEntry = resolveLocationEntry({ country, region, city, variant }, 'fa');
  ...
  return localeEntry
    ...
    : localizeMetadata(locationMetadata(entry), 'fa', entry.path);
}

export default async function FaVariantPage({ params }: FaVariantPageProps) {
  const { country, region, city, variant } = await params;

...
/**
 * `/th/location` — generated locale location hub (TASK-448).
 *
 ...
 * while canonical + hreflang localize to `/th/location` with
...
export const metadata: Metadata = (() => {
  ...
  return localizeMetadata(locationMetadata(entry), 'th', entry.path);
})();

export default async function ThLocationHubPage() {
  const entry = hubEntry();
  if (!entry) {
    return null;
  }
  const data = buildLocationViewData(entry, 'th', await getServerCountry());

...
/**
 * `/th/glossary` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/glossary/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/th/glossary' })`, breadcrumb `Home` → `/th`,
 ...
 * `/th/glossary` and `alternates.languages` `th` + `en` + `x-default` → EN canonical.
 */
...
export default function ThGlossaryPage() {
  return (
    <>
      <GlossaryHubView />
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/th' },
          { name: 'Glossary', path: '/th/glossary' },
        ])}
      />
    </>
  );

...
/**
 * `/pt-BR/network` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/network/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/pt-BR/network' })`, breadcrumb `Home` → `/pt-BR`,
 ...
 * `/pt-BR/network` and `alternates.languages` `pt-BR` + `en` + `x-default` → EN canonical.
 */
...
export default function PtBRNetworkPage() {
  return (
    <>
      ...
      <JsonLd
        data={breadcrumbList([
          ...
          { name: 'Network', path: '/pt-BR/network' },
        ])}
      />
    </>
  );

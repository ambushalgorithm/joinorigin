...
/**
 * `/pt-BR/location/[country]/[region]/[city]/[variant]` — generated locale location
 ...
 * (`resolveLocationEntry(params, 'pt-BR')`), EN entry otherwise —
 ...
 * `/pt-BR/location/[country]/[region]/[city]/[variant]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface PtBRVariantPageProps {
  params: Promise<{ country: string; region: string; city: string; variant: string }>;
}

export async function generateMetadata({ params }: PtBRVariantPageProps): Promise<Metadata> {
  const { country, region, city, variant } = await params;
  const localeEntry = resolveLocationEntry({ country, region, city, variant }, 'pt-BR');
  ...
  return localeEntry
    ...
    : localizeMetadata(locationMetadata(entry), 'pt-BR', entry.path);
}

export default async function PtBRVariantPage({ params }: PtBRVariantPageProps) {
  const { country, region, city, variant } = await params;

...
/**
 * `/pt-BR/glossary` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/glossary/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/pt-BR/glossary' })`, breadcrumb `Home` → `/pt-BR`,
 ...
 * `/pt-BR/glossary` and `alternates.languages` `pt-BR` + `en` + `x-default` → EN canonical.
 */
...
export default function PtBRGlossaryPage() {
  return (
    <>
      <GlossaryHubView />
      <JsonLd
        data={breadcrumbList([
          ...
          { name: 'Glossary', path: '/pt-BR/glossary' },
        ])}
      />
    </>
  );

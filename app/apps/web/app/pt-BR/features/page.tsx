...
/**
 * `/pt-BR/features` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/features/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/pt-BR/features' })`, breadcrumb `Home` → `/pt-BR`,
 ...
 * `/pt-BR/features` and `alternates.languages` `pt-BR` + `en` + `x-default` → EN canonical.
 */
...
export default function PtBRFeaturesPage() {
  return (
    <>
      <FeaturesView />
      <JsonLd
        data={breadcrumbList([
          ...
          { name: 'Features', path: '/pt-BR/features' },
        ])}
      />
    </>
  );

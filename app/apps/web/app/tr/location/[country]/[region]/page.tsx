...
/**
 * `/tr/location/[country]/[region]` — generated locale location
 ...
 * `/tr/location/[country]/[region]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface TrRegionPageProps {
  params: Promise<{ country: string; region: string }>;
}

export async function generateMetadata({ params }: TrRegionPageProps): Promise<Metadata> {
  const { country, region } = await params;
  const localeEntry = resolveLocationEntry({ country, region }, 'tr');
...
export default async function TrRegionPage({ params }: TrRegionPageProps) {
  const { country, region } = await params;

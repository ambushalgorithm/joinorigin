...
/**
 * `/tr/location/[country]/[region]/[city]` — generated locale location
 ...
 * `/tr/location/[country]/[region]/[city]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface TrCityPageProps {
  params: Promise<{ country: string; region: string; city: string }>;
}

export async function generateMetadata({ params }: TrCityPageProps): Promise<Metadata> {
  const { country, region, city } = await params;
  const localeEntry = resolveLocationEntry({ country, region, city }, 'tr');
...
export default async function TrCityPage({ params }: TrCityPageProps) {
  const { country, region, city } = await params;

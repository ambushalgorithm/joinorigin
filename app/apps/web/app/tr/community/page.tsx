...
/**
 * `/tr/community` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/community/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/tr/community' })`, breadcrumb `Home` → `/tr`,
 ...
 * `/tr/community` and `alternates.languages` `tr` + `en` + `x-default` → EN canonical.
 */
  ...
  return (
    <>
      ...
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/tr' },
          { name: 'Community', path: '/tr/community' },
        ])}
      />
    </>
  );

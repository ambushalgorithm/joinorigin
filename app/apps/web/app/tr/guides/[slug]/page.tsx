 ...
 * `/tr/guides/[slug]` with `x-default` → EN canonical.
 */
...
interface TrGuidePageProps {
  params: Promise<{ slug: string }>;
}

export async function generateMetadata({ params }: TrGuidePageProps): Promise<Metadata> {
  const { slug } = await params;
  const entry = guidePageEntry(slug, 'tr') ?? guidePageEntry(slug);
  if (!entry) {
    return {};
  }
  return guidePageMetadata(entry, 'tr');
}

export default async function TrGuidePage({ params }: TrGuidePageProps) {
  const { slug } = await params;
  const page = guidePageForLocale(slug, 'tr') ?? guidePageForLocale(slug);
  if (!page) {
    notFound();
  }

...
/**
 * `/tr/glossary` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/glossary/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/tr/glossary' })`, breadcrumb `Home` → `/tr`,
 ...
 * `/tr/glossary` and `alternates.languages` `tr` + `en` + `x-default` → EN canonical.
 */
...
export default function TrGlossaryPage() {
  return (
    <>
      <GlossaryHubView />
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/tr' },
          { name: 'Glossary', path: '/tr/glossary' },
        ])}
      />
    </>
  );

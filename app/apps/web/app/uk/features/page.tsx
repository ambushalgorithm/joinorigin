...
/**
 * `/uk/features` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/features/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/uk/features' })`, breadcrumb `Home` → `/uk`,
 ...
 * `/uk/features` and `alternates.languages` `uk` + `en` + `x-default` → EN canonical.
 */
...
export default function UkFeaturesPage() {
  return (
    <>
      <FeaturesView />
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/uk' },
          { name: 'Features', path: '/uk/features' },
        ])}
      />
    </>
  );

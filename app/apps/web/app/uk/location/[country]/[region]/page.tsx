...
/**
 * `/uk/location/[country]/[region]` — generated locale location
 ...
 * `/uk/location/[country]/[region]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface UkRegionPageProps {
  params: Promise<{ country: string; region: string }>;
}

export async function generateMetadata({ params }: UkRegionPageProps): Promise<Metadata> {
  const { country, region } = await params;
  const localeEntry = resolveLocationEntry({ country, region }, 'uk');
...
export default async function UkRegionPage({ params }: UkRegionPageProps) {
  const { country, region } = await params;

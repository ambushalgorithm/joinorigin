...
/**
 * `/uk/location/[country]/[region]/[city]/[variant]` — generated locale location
 ...
 * `/uk/location/[country]/[region]/[city]/[variant]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface UkVariantPageProps {
  params: Promise<{ country: string; region: string; city: string; variant: string }>;
}

export async function generateMetadata({ params }: UkVariantPageProps): Promise<Metadata> {
  const { country, region, city, variant } = await params;
  const localeEntry = resolveLocationEntry({ country, region, city, variant }, 'uk');
...
export default async function UkVariantPage({ params }: UkVariantPageProps) {
  const { country, region, city, variant } = await params;

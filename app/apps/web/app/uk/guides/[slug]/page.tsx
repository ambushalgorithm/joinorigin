 ...
 * `/uk/guides/[slug]` with `x-default` → EN canonical.
 */
...
interface UkGuidePageProps {
  params: Promise<{ slug: string }>;
}

export async function generateMetadata({ params }: UkGuidePageProps): Promise<Metadata> {
  const { slug } = await params;
  const entry = guidePageEntry(slug, 'uk') ?? guidePageEntry(slug);
  if (!entry) {
    return {};
  }
  return guidePageMetadata(entry, 'uk');
}

export default async function UkGuidePage({ params }: UkGuidePageProps) {
  const { slug } = await params;
  const page = guidePageForLocale(slug, 'uk') ?? guidePageForLocale(slug);
  if (!page) {
    notFound();
  }

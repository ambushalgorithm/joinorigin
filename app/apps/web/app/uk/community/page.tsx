...
/**
 * `/uk/community` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/community/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/uk/community' })`, breadcrumb `Home` → `/uk`,
 ...
 * `/uk/community` and `alternates.languages` `uk` + `en` + `x-default` → EN canonical.
 */
...
export default function UkCommunityPage() {
  return (
    <>
      ...
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/uk' },
          { name: 'Community', path: '/uk/community' },
        ])}
      />
    </>
  );

...
/**
 * `/hi/location/[country]/[region]/[city]` — generated locale location
 ...
 * `/hi/location/[country]/[region]/[city]` (`x-default` → EN
 * canonical). Rendered per-request: the root layout reads `headers()`,
 * so SSG/ISR would crash with DYNAMIC_SERVER_USAGE.
 */
export const dynamic = 'force-dynamic';

interface HiCityPageProps {
  params: Promise<{ country: string; region: string; city: string }>;
}

export async function generateMetadata({ params }: HiCityPageProps): Promise<Metadata> {
  const { country, region, city } = await params;
  const localeEntry = resolveLocationEntry({ country, region, city }, 'hi');
...
export default async function HiCityPage({ params }: HiCityPageProps) {
  const { country, region, city } = await params;

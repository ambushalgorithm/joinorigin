 ...
 * `/hi/guides/[slug]` with `x-default` → EN canonical.
 */
...
interface HiGuidePageProps {
  params: Promise<{ slug: string }>;
}

export async function generateMetadata({ params }: HiGuidePageProps): Promise<Metadata> {
  const { slug } = await params;
  const entry = guidePageEntry(slug, 'hi') ?? guidePageEntry(slug);
  if (!entry) {
    return {};
  }
  return guidePageMetadata(entry, 'hi');
}

export default async function HiGuidePage({ params }: HiGuidePageProps) {
  const { slug } = await params;
  const page = guidePageForLocale(slug, 'hi') ?? guidePageForLocale(slug);
  if (!page) {
    notFound();
  }

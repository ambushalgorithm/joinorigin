...
/**
 * `/vi/community` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/community/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/vi/community' })`, breadcrumb `Home` → `/vi`,
 ...
 * `/vi/community` and `alternates.languages` `vi` + `en` + `x-default` → EN canonical.
 */
...
export default function ViCommunityPage() {
  return (
    <>
      ...
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/vi' },
          { name: 'Community', path: '/vi/community' },
        ])}
      />
    </>
  );

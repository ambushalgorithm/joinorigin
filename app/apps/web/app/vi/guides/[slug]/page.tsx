 ...
 * `/vi/guides/[slug]` with `x-default` → EN canonical.
 */
...
export async function generateMetadata({ params }: ViGuidePageProps): Promise<Metadata> {
  ...
  const entry = guidePageEntry(slug, 'vi') ?? guidePageEntry(slug);
  if (!entry) {
    return {};
  }
  return guidePageMetadata(entry, 'vi');
}

export default async function ViGuidePage({ params }: ViGuidePageProps) {
  const { slug } = await params;
  const page = guidePageForLocale(slug, 'vi') ?? guidePageForLocale(slug);
  if (!page) {
    notFound();
  }

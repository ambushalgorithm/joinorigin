...
/**
 * `/vi/glossary` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/glossary/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/vi/glossary' })`, breadcrumb `Home` → `/vi`,
 ...
 * `/vi/glossary` and `alternates.languages` `vi` + `en` + `x-default` → EN canonical.
 */
...
export default function ViGlossaryPage() {
  return (
    <>
      <GlossaryHubView />
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/vi' },
          { name: 'Glossary', path: '/vi/glossary' },
        ])}
      />
    </>
  );

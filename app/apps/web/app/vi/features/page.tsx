...
/**
 * `/vi/features` — generated locale wrapper (TASK-448).
 *
 * Mirrors the EN `app/features/page.tsx`
 * wrapper with the locale-prefixed path: `createMetadata({ path:
 * '/vi/features' })`, breadcrumb `Home` → `/vi`,
 ...
 * `/vi/features` and `alternates.languages` `vi` + `en` + `x-default` → EN canonical.
 */
...
export default function ViFeaturesPage() {
  return (
    <>
      <FeaturesView />
      <JsonLd
        data={breadcrumbList([
          { name: 'Home', path: '/vi' },
          { name: 'Features', path: '/vi/features' },
        ])}
      />
    </>
  );

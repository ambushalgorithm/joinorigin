...
const content: GuideContent = {
  ...
  intro: [
    'Every startup — whether it is still a concept on paper or already operating with customers — needs people more than it needs capital: a founder who can build it, a team that can ship it, and users who will test it. A startup no one can find gathers none of those. Publishing the concept as a findable idea page, then opening a room where the conversation can happen, is the honest first step of building a startup — not the deck, not the logo, not the pitch — and it works just as well for an existing company that wants more believers, co-founders, and testers around what it is building.',
    ...
    'This guide walks through publishing a startup concept like an operator — whether the concept is brand new or the company is already running: compressing the concept into one sentence, writing the page with honest signals, publishing it and opening the room, sharing it with founder communities, inviting early believers and testers, running structured conversations, using the room to form a trial team, and feeding the room into the feed as the concept is validated.',
  ],
  dataPoints: [

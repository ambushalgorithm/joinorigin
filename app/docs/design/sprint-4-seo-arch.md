...
│   ├── llms.txt.ts            # → /llms.txt     (MetadataRoute? route handler — see §3.7)
│   ├── about/
...
- `sitemap.ts`, `robots.ts`, the header/footer nav (fe-menu-pages), and
  metadata builders all read from `ROUTES` — a page added to `ROUTES` is
  automatically in the sitemap and canonical links. **No duplicated URL lists.**
- The exact route set (and whether home is `'/'`) is finalized by
  `sprint-4-discovery.md`; `ROUTES` reflects that list.
...
| Builder | Schema | Where mounted |
|---|---|---|
| `organization()` | `Organization` — name, url, logo, sameAs (socials if known) | `layout.tsx` (once, site-wide) |
| `website()` | `WebSite` — name, url, potentialAction `SearchAction` if a search route exists (skip otherwise) | `layout.tsx` (once) |
| `breadcrumbList(items)` | `BreadcrumbList` — path → `ItemList` entries with `@id` + `name` | every menu page (server wrapper) |
| `faqPage(questions)` | `FAQPage` — question/answer pairs from `sprint-4-discovery.md` FAQ | home (if discovery recommends) or contact/community |
...
- **Acceptance (e2e):** `GET /sitemap.xml` → 200, XML containing
  `https://<site-url>/about`, `/features`, `/community`, `/contact`, `/`.
...
Deliverable depends on the discovery recommendation (fe-seo follows
`sprint-4-discovery.md`; recommended **yes** for the LLM-crawler strategy).

Design when included — served at `/llms.txt` via a route handler with
`Content-Type: text/plain; charset=utf-8` and a generated markdown doc:
...
- `GET /` and each menu page: exactly one `h1`; `h2`s present; `<main>` +
  `<nav>` landmarks; canonical matches URL; JSON-LD `@type` includes
...
| llms.txt | Recommended; plain-text route handler from `LLMS_ENTRIES` | Cheap, high-value LLM-crawler entry point |
